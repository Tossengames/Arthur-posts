...
GAMING_RSS_FEEDS = [
    ...
    "https://www.ign.com/feeds/rss",  # IGN
    ...
    "https://www.theverge.com/games/rss/index.xml",  # The Verge Gaming
]
...
def post_gaming_news():
    ...
    try:
        ...
        if not entries:
            print("❌ No gaming news entries found from any RSS feed.")
            fallback_message = (
                "🎮 لا توجد أخبار ألعاب رئيسية للإبلاغ عنها الآن! "
                "ما هي آخر لعبة لعبتها؟ شاركنا تجربتك! 👇 "
                "#ألعاب_فيديو #ألعاب_إندي #أخبار_الجيمز"
            )
        ...
        for entry in entries[:5]:  # Use top 5 entries
            ...
            if not title or not summary or title.lower().startswith('no title'):
                print(f"[Gaming News] Skipping malformed entry: Title='{title}', Summary='{summary}'")
                continue
    ...
    fallback_hashtags = "#ألعاب_فيديو #ألعاب_إندي #أخبار_الجيمز #مجتمع_الألعاب"
