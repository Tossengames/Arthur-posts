...
def post_soccer_news():
    ...
    try:
        ...
        if not entries:
            ...
            fallback_message = (
                "⚽ No major soccer news headlines to report right now! "
                "The football world might be quiet, but the action is always around the corner! "
                "What's your favorite soccer moment of all time? Let us know! 👇 "
                "#Soccer #Football #SoccerNews"
            )
        ...
        keyword_hashtags = " ".join([f"#{keyword}" for keyword in generated_keywords])

        prompt = (
            "Summarize the following soccer/football news headlines in an engaging Facebook post "
            "for soccer fans. Use a casual, enthusiastic tone. "
            "Format the post with paragraphs and strategic emojis to improve readability. "
            "Conclude with a strong call to action asking fans to like, share, and comment on their thoughts, "
            "and end with 3–4 relevant hashtags. "
            "Do not include links in the final post. Here's the news:\n\n" + raw_combined +
            ("\n\nAlso, consider these keywords for additional relevant hashtags: " +
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "candidates" in data and data["candidates"]:
                    ...
                    print("[Gemini Soccer News] Successfully generated and posted content.")
                    return
                else:
    ...
    if clean_posts:
        fallback_message = (
            "⚽ Latest Soccer News:\n\n" +
            "\n\n".join(clean_posts) +
            "\n\nWhat are your thoughts on these headlines? Like, share, and comment below! 👇"
        )
    else:
        fallback_message = (
            "⚽ No major soccer news headlines to report right now! "
    ...
    fallback_hashtags = "#Soccer #Football #SoccerNews #FootballFans"
